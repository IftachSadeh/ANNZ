...
// ============
  ...
  public:
    ...
    inline TString versionTag()     { return (TString)"versionTag"; };
    ...
    void  checkLock(TString aName) {
      VERIFY(LOCATION,(TString)"Tried to change \""+aName+"\" in OptMaps which is locked",!isLocked);
      return;
    }

    void                 checkName(TString messageTag, TString aName);
    void                 copyOpt(TString aName, OptMaps * inObj, Log::LOGtypes log = Log::DEBUG_2);
    ...
    // template <typename T>
    // inline void SetOptX(TString aName, T input) {
    //   TString aType(GetOptType(aName));
    //   if     (aType == "B") SetOptB(aName,input); else if(aType == "I") SetOptI(aName,input);
    //   else if(aType == "F") SetOptF(aName,input); else if(aType == "C") SetOptC(aName,input);
    //   return;
    // }
    ...
    inline TString  GetOptType(TString aName) {
      TString type("");
    ...
    // ===========================================================================================================
    template <typename T>
    void printMap(Map <TString,T> & input, TString message = "", int nPrintRow = 0, int width = 0) {
    // =============================================================================================
    ...
    // ===========================================================================================================
    template <typename T>
    void getElePattern(Map <TString,T> & input, vector <TString> & optVarV, TString pattern, bool ignorCase = false) {
    // ===============================================================================================================
      for(typename Map <TString,T>::iterator itr = input.begin(); itr!=input.end(); ++itr) {
