...
// ==========
  public:  
    ...
    inline bool     isDirFile(TString inputName) {
      assert(!inputName.Contains("*")); 
      struct stat sb; return ((stat(inputName, &sb) == 0 && S_ISDIR(sb.st_mode)));
    };
    
    inline TString  nextTreeFriendName(TTree * tree) {
    ...
    void            getSetActiveTreeBranches(TTree * tree, vector < pair<TString,bool> > & branchNameStatusV,
                                             TString getSet = "", bool verbose = false);
    ...
    int     drawTree(TTree * tree = NULL, TString drawExprs = "", TString wgtCut = "");
