...
// ============
  ...
  private:
    ...
    TString         treeReadName, treeWriteName;
    int             nTreeInChain;
  ...
  public: 
    ...
    void            createTreeBranches(TTree * tree = NULL, TString prefix = "",
                                       TString postfix = "", vector <TString> * excludedBranchNames = NULL);
    ...
    // interfaces to set variables of whatever types
    void SetVarAuto(TString aName, TString input);
    void SetVarAuto(vector< pair<TString,TString> > & inputV);
    void SetVarAuto(vector<TString> & nameV, vector<TString> & inputV);
    ...
    inline Long64_t  GetVarI(TString aName) {
      Long64_t val (0);
      if     (HasVarI_(aName)) val = static_cast<Long64_t> (GetVarI_(aName));
      else if(HasVarS_(aName)) val = static_cast<Long64_t> (GetVarS_(aName));
      else if(HasVarL_(aName)) val = GetVarL_(aName);
      else                     AsrtVar(false,aName+" (GetVarI)");
      return val;
    };
    inline ULong64_t GetVarU(TString aName) {
      ULong64_t val(0);
      if     (HasVarUI_(aName)) val = static_cast<ULong64_t>(GetVarUI_(aName));
      else if(HasVarUS_(aName)) val = static_cast<ULong64_t>(GetVarUS_(aName));
      else if(HasVarUL_(aName)) val = GetVarUL_(aName);
      else                      AsrtVar(false,aName+" (GetVarU)");
      return val;
    };
    inline Double_t  GetVarF(TString aName) {
      Double_t val (0);
      if     (HasVarF_ (aName)) val = static_cast<Double_t> (GetVarF_ (aName));
      else if(HasVarD_ (aName)) val = GetVarD_ (aName);
      else                      AsrtVar(false,aName+" (GetVarF)");
      return val;
    };
    ...
    inline bool HasVar  (TString aName) {
      return (HasVarB(aName)||HasVarC(aName)||HasVarI(aName)||HasVarU(aName)||HasVarF(aName)||HasForm(aName));
    }
    ...
    inline TString GetVarType(TString aName) {
      ...
      if(HasVarB_ (aName)) { type += "B";  nHas++; }
      if(HasVarC_ (aName)) { type += "C";  nHas++; }
      if(HasVarS_ (aName)) { type += "S";  nHas++; }
      if(HasVarI_ (aName)) { type += "I";  nHas++; } 
      if(HasVarL_ (aName)) { type += "L";  nHas++; }
      if(HasVarUS_(aName)) { type += "US"; nHas++; }
      if(HasVarUI_(aName)) { type += "UI"; nHas++; }
      if(HasVarUL_(aName)) { type += "UL"; nHas++; }
      if(HasVarF_ (aName)) { type += "F";  nHas++; }
      if(HasVarD_ (aName)) { type += "D";  nHas++; }
      if(HasForm_ (aName)) { type += "FM"; nHas++; }

      if(nHas != 1) {
        AsrtVar(nHas < 2,(TString)"multiple definitions - "+aName);
        AsrtVar(nHas > 0,(TString)"undefined var - "+aName);
      }
      return type;
    }
  ...
  private:
    ...
    void AsrtVar(bool cond, TString name) { 
      VERIFY(LOCATION,(TString)" - VarMaps("+name+") tried to get non existing Var (\""+name+"\") ...",cond);
      return;
    }
    void checkLock(TString aName) { 
      if(dynamic_cast<TTree*>(treeRead)||dynamic_cast<TTree*>(treeWrite)) {
        AsrtVar(false,(TString)" - VarMaps("+name+") tried to add \""+aName+"\" with tree already defined !");
      }
      return;
    }
};
